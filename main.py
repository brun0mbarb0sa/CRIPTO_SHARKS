...
def fetch_binance_futures_klines():
    ...
    df[['open','high','low','close','volume','taker_buy_base_volume']] = df[['open','high','low','close','volume','taker_buy_base_volume']].astype(float)
...
def normalize_volume(df):
    ...
    df[['timestamp', 'open', 'high', 'low', 'close', 'volume', 'taker_buy_base_volume', 'volume_usd']].to_csv(f"{PROCESSED_DIR}/binance_btcusdt_normalized.csv", index=False)
    return df
...
def exportar_para_json(df_ind):
    ...
    colunas = [c for c in df_ind.columns if c not in ['timestamp','open','high','low','close','volume','volume_usd','taker_buy_base_volume','time']]
    result = {col: [{"time": int(t), "value": float(v)} for t, v in zip(df_ind['time'], df_ind[col]) if pd.notna(v)] for col in colunas}
