...
def calcular_indicadores(df, config):
    ...
    df = calcular_volume_delta(df)
    return df

def calcular_volume_delta(df):
    df = df.copy()
    df['volume_delta'] = df['taker_buy_base_volume'] - (df['volume'] - df['taker_buy_base_volume'])
    df['volume_delta_sma'] = df['volume_delta'].rolling(window=20).mean()
    return df
